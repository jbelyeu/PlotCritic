...
def add_annotate(parent_parser):
    parser = parent_parser.add_parser(
        "annotate",
        help="annotate a VCF file with scoring results, "
    ...
    parser.add_argument(
        "-a",
        "--annotated_outfile",
        help="name for new annotated variant file",
        required=True,
    )
    parser.add_argument(
        "-o",
        "--operation",
        help="summarizing operation for scores. "
        + "Options are mean, median, standard deviation (stdev), mode, max, min",
        required=False,
        choices=["mean", "median", "stdev", "mode", "max", "min"],
    )
    parser.add_argument(
        "-n",
        "--number_map",
        help="ordered list of number values for answers, used for summarizing results. "
        + "Required if --operation is included. "
        + "Example: if the curation answers in order are: 'yes', 'no', 'maybe' => 1,0,0.5 ",
        type=comma_list,
        required=False,
    )
...
def annotate(args, parser):
        ...
        for line in scores:
            if line[0] == "#":
                ...
                if args.number_map and len(answers) != len(args.number_map):
                    print(
                        "Error: count of curation answers does not "
                        + "equal count of number values in number_map"
                    )
    ...
    vcf.add_info_to_header(
        {
            "ID": "SVPD",
            "Description": "Details of SV-plaudit scorer count and "
            + "scores in the format COUNT|SCORE1,SCORE2,SCOREN. Answers the question: `"
            + question
            + "` Available answers were as follows: `"
            + "`; `".join(answers)
            + "`",
            "Type": "Character",
            "Number": "1",
    ...
    vcf.add_info_to_header(
        {
            "ID": "SVP",
            "Description": "SV-plaudit curation score, the "
            + args.operation
            + " of scores for that entry where the values of the following curation answers: `"
            + "`; `".join(answers)
            + "` are "
            + ",".join(args.number_map),
            "Type": "Float",
            "Number": "1",
        }
    )
    ...
    for variant in vcf:
        if variant.INFO.get("END"):
            ...
            if key in scored_variants:
                vcf_annotation = (
                    str(scored_variants[key]["score_fields"]["scorer_count"]) + "|"
                )
                for answer in answers:
                    vcf_annotation += (
                        str(scored_variants[key]["score_fields"][answer]) + ","
                    )
                ...
                if args.operation:
                    ...
                    for i in range(len(score_counts)):
                        score_values += [float(args.number_map[i])] * int(
                            score_counts[i]
                        )
                    curation_score = 0
                    try:
                        if args.operation == "max" or args.operation == "min":
                            ...
                            curation_score = getattr(statistics, args.operation)(
                                score_values
                            )
                    except: #TODO set exception type
                        print(
                            "Warning: failed to perform specified operation on entry:"
                        )
                        print(variant)
                        continue
